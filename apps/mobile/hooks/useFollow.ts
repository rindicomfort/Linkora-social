...
export const useFollow = (targetAddress: string) => {
  ...
  const updateCache = async (newStatus: boolean) => {
    ...
    const previousCount = queryClient.getQueryData(['followerCount', targetAddress]) as number || 0;

    queryClient.setQueryData(['isFollowing', me, targetAddress], newStatus);
    queryClient.setQueryData(['followerCount', targetAddress], newStatus ? previousCount + 1 : Math.max(0, previousCount - 1));
